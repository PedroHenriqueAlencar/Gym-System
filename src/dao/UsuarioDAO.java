...
public class UsuarioDAO {
    ...
    public void editar(Usuario usuario, int mode){
        String sql;
        if(mode == 0)
            sql = "UPDATE login SET login=?, senha=?, nivel=? WHERE ra=?";
        else
            sql = "UPDATE login SET login=?, senha=?, nivel=? WHERE login=?";
        try{
            PreparedStatement stmt = this.conn.prepareStatement(sql);
            ...
            if(mode == 0)
                stmt.setInt(4, usuario.getRa());        //atualiza os campos no RA informado
            else
                stmt.setString(4, usuario.getLogin());     //atualiza os campos no RA informado
            stmt.execute();                             //roda o comando no mysql
        }
        catch(SQLException e){
            //se deu erro na hora de conectar
